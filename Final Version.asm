...
.data
	...
	soundDropoff BYTE "dropoff.wav", 0
	soundCrash BYTE "crash.wav", 0
	soundGameOver BYTE "gameover.wav", 0
	soundGameLost BYTE "gamelost.wav", 0
	soundMove BYTE "move.wav", 0
	soundEngine BYTE "engine.wav", 0
...
; External API function prototype
PlaySoundA PROTO,
	pszSound:PTR BYTE,
	hmod:DWORD,
	fdwSound:DWORD
...
;--------------------------------------------------------------------------------------------
; Play a sound asynchronously (doesn't block execution)
; Input: EDX = pointer to sound filename
;--------------------------------------------------------------------------------------------
PlaySoundAsync PROC
	push edx
	push eax
...
;--------------------------------------------------------------------------------------------
; Play a sound synchronously (waits for sound to finish)
; Input: EDX = pointer to sound filename
;--------------------------------------------------------------------------------------------
PlaySoundSync PROC
	push edx
	...
	INVOKE PlaySoundA, edx, 0, SND_FILENAME + SND_SYNC
...
PlaySoundLoop PROC
	...
	INVOKE PlaySoundA, edx, 0, SND_FILENAME + SND_ASYNC + SND_LOOP
...
EndGame:
	...
	call StopSound ; Kill engine sound FIRST

	mov edx, OFFSET soundGameOver
	call PlaySoundAsync ; Play game over sound asynchronously
